...
def BackupDesktopWallPaper(targetDir):
    ...
    # 构造备份文件名
    sub_dir_name = "Desktop"
    newFilename, subDir = ComposeNewFilename(desktopWallPaperFilename, sub_dir_name)
    ...
    for fileEntry in Path(wallpaperDir).iterdir():
        if fileEntry.is_file():
            ...
            if extension != ".jpg":
                ...
                # 构造新的文件名
                sub_dir_name = "WallPaper"
                newFilename, subDir = ComposeNewFilename(fileEntry, sub_dir_name)
                # 创建目标目录，格式为当前月份
                backupDir = targetDir / subDir
                backupDir.mkdir(exist_ok=True)
                backupDir = backupDir / sub_dir_name
